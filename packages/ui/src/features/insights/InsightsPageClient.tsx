...
export default function InsightsPageClient({
  ...
  const [connectionReady, setConnectionReady] = useState(!connectionName);
  const intervalRef = useRef<ReturnType<typeof setInterval> | null>(null);

  const fetchInsights = useCallback(async () => {
    if (connectionName && activeConnection !== connectionName) {
      return;
    }
    setLoading(true);
    setError(null);
  ...
  }, [activeConnection, connectionName, days]);
  ...
  useEffect(() => {
    ...
    switchConnection(connectionName)
      .catch((err) => {
        if (!cancelled) {
  ...
  }, [connectionName, switchConnection]);
  ...
  return (
    <div className="space-y-6">
      ...
      {(loading || !connectionReady) && !analysis && (
        <div className="text-center py-12 text-gray-500">
          <p>{connectionReady ? "Analyzing traces..." : "Preparing connection insights..."}</p>
        </div>
      )}
