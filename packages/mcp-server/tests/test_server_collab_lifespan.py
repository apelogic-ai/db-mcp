...
import pytest
from db_mcp_knowledge.collab.manifest import CollabManifest, CollabMember, CollabSyncConfig
from db_mcp_knowledge.collab.sync import SyncResult
...
class TestServerCollabLifespan:
    ...
    async def test_pulls_on_startup_for_collaborator(self):
        ...
        with (
            ...
            patch(
                "db_mcp_knowledge.collab.manifest.get_member",
            ...
            from db_mcp_server.server import server_lifespan
    ...
    async def test_pushes_on_shutdown_for_collaborator(self):
        ...
        with (
            ...
            patch(
                "db_mcp_knowledge.collab.sync.collaborator_push",
    ...
    async def test_no_manifest_skips_collab(self):
        ...
        with (
            ...
            patch("db_mcp_knowledge.collab.sync.collaborator_pull") as mock_pull,
            patch("db_mcp_knowledge.collab.sync.collaborator_push") as mock_push,
        ):
            mock_store.return_value.start_cleanup_loop = AsyncMock()
            mock_store.return_value.stop_cleanup_loop = AsyncMock()
