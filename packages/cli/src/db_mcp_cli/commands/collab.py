...
from db_mcp_cli.init_flow import _attach_repo, _auto_register_collaborator
from db_mcp_cli.utils import CONFIG_FILE, console, load_config, save_config
...
def collab_init():
    ...
    from db_mcp_knowledge.collab.manifest import (
        CollabManifest,
        add_member,
    ...
    user_name = get_user_name_from_config(CONFIG_FILE)
    if not user_name:
        user_name = click.prompt("Your name (used for branch names and attribution)")
        set_user_name_in_config(user_name, CONFIG_FILE)
...
def collab_sync(connection: str | None):
    ...
    from db_mcp_knowledge.collab.sync import full_sync
...
def collab_merge(connection: str | None):
    ...
    """
    from db_mcp.traces import get_user_id_from_config
    from db_mcp_knowledge.collab.manifest import get_role
    from db_mcp_knowledge.collab.merge import master_merge_all
...
def collab_prune(connection: str | None):
    ...
    """
    from db_mcp_knowledge.collab.merge import prune_merged_branches
    from db_mcp_knowledge.git_utils import git
...
def collab_daemon(connection: str | None, interval: int | None):
    ...
    from db_mcp.traces import get_user_id_from_config
    from db_mcp_knowledge.collab.background import CollabSyncLoop
    from db_mcp_knowledge.collab.manifest import (
        get_member,
        get_user_name_from_config,
        load_manifest,
