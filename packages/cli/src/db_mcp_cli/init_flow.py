...
from db_mcp_cli.agent_config import _configure_agents
from db_mcp_cli.connection import (
    _prompt_and_save_api_connection,
    _prompt_and_save_database_url,
    get_connection_path,
)
from db_mcp_cli.discovery import _run_discovery_with_progress
...
def _attach_repo(name: str, connection_path: Path, git_url: str):
    ...
    """
    from db_mcp_knowledge.collab.classify import classify_files
    ...
    if not get_user_name_from_config(CONFIG_FILE):
    ...
    try:
        from db_mcp_knowledge.git_utils import git as _git
...
def _init_greenfield(name: str, template_name: str | None = None):
    ...
    from db_mcp_cli.agent_config import extract_database_url_from_claude_config
    from db_mcp_cli.utils import load_claude_desktop_config
