...
ConnectorRuntimeFactory = Callable[[dict[str, Any], Path], Any]
...
def _builtin_runtime_factory(plugin_id: str) -> ConnectorRuntimeFactory | None:
    if plugin_id == "metabase":
        from db_mcp_data.connector_plugins.builtin.metabase import build_metabase_connector

        return build_metabase_connector
    if plugin_id == "superset":
        from db_mcp_data.connector_plugins.builtin.superset import build_superset_connector
