...
class TestSQLConnector:
    ...
    def test_test_connection_delegates(self, connector, mock_engine):
        ...
        with patch("db_mcp_data.connectors.sql.db_test_connection") as mock_test:
    ...
    def test_get_catalogs_delegates(self, connector):
        ...
        with patch("db_mcp_data.connectors.sql.db_get_catalogs") as mock:
    ...
    def test_get_schemas_delegates(self, connector):
        ...
        with patch("db_mcp_data.connectors.sql.db_get_schemas") as mock:
    ...
    def test_get_tables_delegates(self, connector):
        ...
        with patch("db_mcp_data.connectors.sql.db_get_tables") as mock:
            mock.return_value = [
    ...
    def test_get_columns_delegates(self, connector):
        ...
        with patch("db_mcp_data.connectors.sql.db_get_columns") as mock:
            mock.return_value = [
    ...
    def test_get_table_sample_delegates(self, connector):
        ...
        with patch("db_mcp_data.connectors.sql.db_get_table_sample") as mock:
    ...
    def test_execute_sql_delegates(self, connector, mock_engine):
        ...
        with patch("db_mcp_data.connectors.sql.get_engine", return_value=mock_engine):
            mock_conn = MagicMock()
...
class TestGetConnector:
    ...
    def test_get_connector_returns_sql_by_default(self, tmp_path, monkeypatch):
    ...
    def test_get_connector_reads_connector_yaml(self, tmp_path, monkeypatch):
    ...
    def test_get_connector_uses_connection_env_database_url(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("DATABASE_URL", "postgresql://wrong-host/wrong-db")
        connector = get_connector(connection_path=tmp_path)
        assert isinstance(connector, SQLConnector)
        assert connector.config.database_url == "trino://user:pass@host:8443/catalog/schema"

    def test_get_connector_env_overrides_connector_yaml_database_url(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("DATABASE_URL", "postgresql://settings-host/settings-db")
        connector = get_connector(connection_path=tmp_path)
        assert isinstance(connector, SQLConnector)
        assert (
            connector.config.database_url
