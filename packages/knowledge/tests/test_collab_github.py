...
from db_mcp_knowledge.collab.github import gh_available, list_prs, open_pr
...
class TestOpenPr:
    @patch("db_mcp_knowledge.collab.github.gh_available", return_value=False)
    def test_returns_none_without_gh(self, _):
        result = open_pr(Path("/fake"), "branch", "title", "body")
        assert result is None

    @patch("subprocess.run")
    @patch("db_mcp_knowledge.collab.github.gh_available", return_value=True)
