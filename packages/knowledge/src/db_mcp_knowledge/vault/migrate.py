...
def detect_legacy_structure(
    *,
    vault_path: Path | str | None = None,
    providers_dir: Path | str | None = None,
    ...
    Args:
        vault_path: Legacy vault directory path (or None for default).
        providers_dir: Legacy providers directory path (or None for default).
        provider_id: Provider identifier for legacy directory resolution.
    ...
    legacy_vault = Path(vault_path) if vault_path else db_mcp_root / "vault"

    # Check for legacy providers path
    legacy_providers = Path(providers_dir) if providers_dir else db_mcp_root / "providers"

    legacy_provider_dir = legacy_providers / provider_id
...
def migrate_to_connection_structure(
    ...
    *,
    ...
    Args:
        ...
        connection_path: Destination connection directory path (required).
        auto_migrate: Whether auto-migration is enabled.
        vault_path: Legacy vault directory path.
        providers_dir: Legacy providers directory path.
        provider_id: Provider identifier for legacy directory resolution.
    ...
    legacy = detect_legacy_structure(
        vault_path=vault_path, providers_dir=providers_dir, provider_id=provider_id
    )
    if not legacy:
    ...
    if connection_path is None:
        raise ValueError("connection_path is required for migration")
...
def migrate_legacy_provider_data(**kwargs) -> dict:
    """Deprecated: Use migrate_to_connection_structure instead."""
    return migrate_to_connection_structure(**kwargs)
