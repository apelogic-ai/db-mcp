...
@pytest.mark.asyncio
async def test_discover_tables_background_emits_insider_bootstrap_after_schema_save(tmp_path):
    """Completed table discovery should enqueue insider bootstrap from the saved schema state."""
    provider_id = "wifimetrics-trino"
    conn_path = tmp_path / "wifimetrics-trino"
    ...
    discovery_id = "test-insider-discovery"
    _discovery_tasks[discovery_id] = {"status": "running"}
    ...
    try:
        with (
            ...
            patch("db_mcp.tools.onboarding.get_insider_supervisor", return_value=supervisor),
        ):
            await _discover_tables_background(discovery_id, provider_id, connection_path=conn_path)

        supervisor.emit_new_connection.assert_awaited_once_with(
            provider_id,
            payload={
