...
class TestDiscoverCommand:
    ...
    def test_discover_no_url_no_connection_no_active(self, tmp_path):
        ...
        runner = CliRunner()
        with (
            patch("db_mcp.cli.main.get_active_connection", return_value="default"),
            patch("db_mcp.cli.main.get_connection_path", return_value=tmp_path / "nonexistent"),
        ):
    ...
    def test_discover_named_connection_not_found(self, tmp_path):
        ...
        runner = CliRunner()
        with patch("db_mcp.cli.main.get_connection_path", return_value=tmp_path / "nonexistent"):
            result = runner.invoke(main, ["discover", "-c", "bogus"])
        assert result.exit_code != 0
        assert "not found" in result.output

    @patch("db_mcp.cli.main.get_connection_path")
