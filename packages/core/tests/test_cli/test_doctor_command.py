...
def test_doctor_json_passes_with_sql_execution(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("db_mcp_cli.commands.core.connection_exists", lambda name: True)
    monkeypatch.setattr("db_mcp_cli.commands.core.get_connection_path", lambda name: conn_path)
    monkeypatch.setattr(
        "db_mcp_cli.commands.core.get_connector",
        lambda connection_path=None: _FakeSQLConnector(),
    )
    monkeypatch.setattr(
        "db_mcp_cli.commands.core.get_connector_capabilities",
...
def test_doctor_fails_for_missing_connection(monkeypatch, tmp_path):
    monkeypatch.setattr("db_mcp_cli.commands.core.connection_exists", lambda name: False)
    monkeypatch.setattr(
        "db_mcp_cli.commands.core.get_connection_path", lambda name: tmp_path / "missing"
    )
