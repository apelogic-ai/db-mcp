...
class _FakeApiConnectorWithFileBase:
    class _FileConfig:
        type = "file"
...
def test_doctor_json_passes_with_sql_execution(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("db_mcp_cli.commands.connection_cmd.connection_exists", lambda name: True)
    monkeypatch.setattr(
        "db_mcp_cli.commands.connection_cmd.get_connection_path", lambda name: conn_path
    )
    monkeypatch.setattr(
        "db_mcp_cli.commands.connection_cmd.get_connector",
        lambda connection_path=None: _FakeSQLConnector(),
    )
    monkeypatch.setattr(
        "db_mcp_cli.commands.connection_cmd.get_connector_capabilities",
...
def test_doctor_prefers_api_connector_type_over_file_base(monkeypatch, tmp_path):
    conn_path = tmp_path / "demo"
    conn_path.mkdir(parents=True, exist_ok=True)
    ...
    monkeypatch.setattr(
        "db_mcp_cli.commands.connection_cmd.get_connector",
        lambda connection_path=None: _FakeApiConnectorWithFileBase(),
    )
    monkeypatch.setattr(
        "db_mcp_cli.commands.connection_cmd.get_connector_capabilities",
        lambda connector: {"supports_sql": False, "supports_async_jobs": False},
    ...
    assert result.exit_code == 0, result.output
    payload = _parse_json(result.output)
    assert payload["connector_type"] == "api"


def test_doctor_fails_for_missing_connection(monkeypatch, tmp_path):
    monkeypatch.setattr("db_mcp_cli.commands.connection_cmd.connection_exists", lambda name: False)
    monkeypatch.setattr(
        "db_mcp_cli.commands.connection_cmd.get_connection_path", lambda name: tmp_path / "missing"
    )
