...
from db_mcp.connector_plugins import (
    ConnectorPlugin,
...
def test_entry_point_connector_plugin_is_loaded(tmp_path):
    ...
    clear_connector_plugin_cache()
    with patch(
        "db_mcp.connector_plugins.registry._iter_plugin_entry_points",
...
def test_get_connector_uses_plugin_runtime_factory(tmp_path):
    ...
    def _factory(connector_data, conn_path, settings):
    ...
    clear_connector_plugin_cache()
    with (
        patch("db_mcp.connectors.get_connector_plugin", return_value=plugin),
        patch("db_mcp.connectors.get_settings") as mock_settings,
    ):
