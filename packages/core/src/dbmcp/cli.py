...
def _init_greenfield(name: str):
    ...
    if LEGACY_VAULT_DIR.exists() or LEGACY_PROVIDERS_DIR.exists():
        ...
        try:
            from dbmcp.vault.migrate import migrate_to_connection_structure
...
def start(connection: str | None):
    ...
    from dbmcp.server import main as server_main
...
def console_cmd(port: int, no_browser: bool):
    ...
    """
    from dbmcp.console import start_console
...
def status(connection: str | None):
    ...
    if claude_config:
        ...
        # Show other servers
        other_servers = [k for k in mcp_servers.keys() if k not in ("dbmeta", "db-mcp")]
        if other_servers:
...
def migrate(dry_run: bool, server: bool):
    ...
    """
    from dbmcp.vault import ensure_connection_structure
    from dbmcp.vault.migrate import (
...
def traces_on():
    ...
    """
    from dbmcp.traces import generate_user_id, get_user_id_from_config
