# db-mcp Docs

Your AI agent doesn't know your data. It hallucinates column names, misses business logic, and forgets everything between sessions. db-mcp fixes that — it sits between your agent and your databases, building a reusable knowledge layer that makes every query smarter than the last.

db-mcp is a local MCP server that gives AI agents structured access to your data sources — with schema awareness, domain context, query validation, and a learning loop that accumulates institutional knowledge over time.

## Choose your journey

- New user (first 10 minutes): [Quickstart](quickstart.md) -> [How-To #1: Get your first useful answer (Playground)](how-to-guides.md#1-get-your-first-useful-answer-playground)
- Data analyst (better SQL quality): [How-To #2: Connect your own database safely](how-to-guides.md#2-connect-your-own-database-safely) -> [How-To #5: Improve answer quality over time](how-to-guides.md#5-improve-answer-quality-over-time-knowledge-loop) -> [Using the Web UI](using-web-ui.md)
- Platform/admin owner (team rollout): [Install and Configuration](install-and-configuration.md) -> [Working with Agents](working-with-agents.md) -> [How-To #6: Collaborate on a shared knowledge vault](how-to-guides.md#6-collaborate-on-a-shared-knowledge-vault) -> [Advanced Topics](advanced-topics.md)

This documentation covers:

- installing and configuring db-mcp
- running it via CLI and Web UI
- connecting agent clients (Claude Desktop, Claude Code, Codex, OpenClaw)
- understanding the MCP tool surface
- using advanced workflows (knowledge management, insights, collaboration)

Start here:

- [Quickstart](quickstart.md)
- [How-To Guides](how-to-guides.md)
- [Install and Configuration](install-and-configuration.md)
- [Using the CLI](using-cli.md)
- [Using the Web UI](using-web-ui.md)
- [Working with Agents](working-with-agents.md)
- [Tools Reference](tools-reference.md)
- [Advanced Topics](advanced-topics.md)
